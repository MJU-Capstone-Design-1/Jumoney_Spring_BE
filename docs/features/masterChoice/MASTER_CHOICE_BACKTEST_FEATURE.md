...
- 연간 재무 스냅샷
    ...
    - 주요 필드: `settlementYearMonth`, `availableDate`, `roe`, `currentEps`, `lastYearEps`, `debtRatio`, `currentSales`,
      `lastYearSales`, `operatingProfit`
...
`MasterChoiceBacktestService`는 백테스트 요청 중 KIS를 호출하지 않고, `stock_candles`,
`master_choice_backtest_financials`, `master_choice_backtest_daily_indicators`를 조회해 평가한다.
KIS 호출은 수동 적재용 `MasterChoiceBacktestDataSyncService`에만 남긴다.
