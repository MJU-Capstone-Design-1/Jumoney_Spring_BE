...
- **로직**:
    ...
    2. 분봉은 KIS 주식당일분봉조회(`FHKST03010200`) 결과를 `09:02`, `09:32`, `10:02` ... `15:32`에 적재하고, `15:40`에 장 마감 보정 동기화를 한 번 더
       수행한다.
    3. 저장 대상은 요청 시각 기준 최근 2분을 제외한 정각/30분 단위 확정 분봉이다.
    4. Redis 미확정 분봉 병합과 `includesRealtime=true` 확장은 아직 미구현이다.
    5. 일/주/월/년봉은 아직 미구현이다.
